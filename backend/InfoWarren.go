...
func HandleInfoJSONWarren(w http.ResponseWriter, r *http.Request) {
	// Set the content type of the response to application/json
	w.Header().Set("Content-Type", "application/json")

	// Loop through the relations to find the one with ID 1
	for _, rel := range warren {
		if rel.ID == 7 {
...
func HandleInfoWarren(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}

	tmpl, err := template.ParseFiles("front-end/InfoGroup/InfoWarrenG.html")
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
